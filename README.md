...
| `library_export({ path, limit? })` | Walk Library → Albums and write the album catalog to `path` as JSON (written atomically; parents created). The albums go to the **file** — the result carries only counts and timings. Can take tens of seconds on a large library. |
